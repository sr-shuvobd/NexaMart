          ...
          {filtered.length === 0 ? (
            ...
            <div className={gridView ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" : "flex flex-col gap-4"}>
              {filtered.map((product) => (
                gridView ? (
                  <div key={product._id} className="group flex flex-col bg-white dark:bg-[#0a0a0a] border border-neutral-200 dark:border-neutral-800 rounded-xl overflow-hidden hover:border-neutral-300 dark:hover:border-neutral-700 transition-colors">
                    <Link href={`/products/${product._id}`} className="block relative aspect-square overflow-hidden bg-neutral-100 dark:bg-neutral-900">
                    ...
                    <div className="p-4 flex flex-col flex-1">
                      ...
                      <div className="flex items-center gap-1.5 mb-3">
                        ...
                        <span className="text-xs font-medium text-neutral-700 dark:text-neutral-300">{product.rating?.toFixed(1) || "5.0"}</span>
                        <span className="text-xs text-neutral-400">({product.reviews || 0})</span>
                      </div>
                  ...
                  <div key={product._id} className="flex gap-4 bg-white dark:bg-[#0a0a0a] border border-neutral-200 dark:border-neutral-800 rounded-xl p-4 hover:border-neutral-300 dark:hover:border-neutral-700 transition-colors relative">
                    <Link href={`/products/${product._id}`} className="w-24 h-24 rounded-lg overflow-hidden shrink-0 bg-neutral-100 dark:bg-neutral-900 relative block">
                      <img src={product.image} alt={product.name} className="w-full h-full object-cover" />
                    </Link>
                    <div className="flex flex-col flex-1 justify-center">
                      <Link href={`/products/${product._id}`} className="text-sm font-medium text-neutral-900 dark:text-white mb-1 hover:text-primary-500">
                        {product.name}
                      </Link>
